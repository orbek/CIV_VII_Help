...
- [ ] All first-release decisions include a concrete action/inspection, a reason it fits the snapshot, evidence dates, and reviewed guide links.
- [ ] Culture advice changes appropriately across the section-6 scenarios and can become more specific using actual available choices.
- [ ] Briefing is useful with Ollama stopped and without a network connection.
- [ ] Facts, advisor judgment, generated interpretation, and player reports are distinguishable.
- [ ] Oracle mode and freshness remain correct through delayed responses, missing sources, and reloads.
- [ ] All observations and tactical contacts are accessible; controls and evidence work by keyboard.
- [ ] Ruleset/guide uncertainty produces conditional or inspection guidance, not invented specifics.
- [ ] README explains the new flow, external guide links, source maintenance, optional player context, and later local persistence.
- [ ] Each completed phase records its verification and remaining limitations; no claims of completion based only on mocks or code inspection.
